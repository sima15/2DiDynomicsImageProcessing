...
public class ElementaryCyclesSearch {
	...
	private List filteredCycles;
	
	@SuppressWarnings({ "rawtypes", "unchecked" })
	private void filterCycles() {
		Map<String,List> cycleMap = new HashMap<String,List>();
		filteredCycles = new ArrayList();
		for (int i = 0; i < cycles.size(); i++) {
			List cycle = (List) cycles.get(i);
			StringBuilder key = new StringBuilder();
			StringBuilder reverseKey = new StringBuilder();
			if (cycle.size() > 2) {
				key.append(cycle.get(0)).append("/");
				for (int j = 1; j < cycle.size(); j++) {
					String node = (String) cycle.get(j);
					key.append(node).append("/");
					StringBuilder newReverseKey = new StringBuilder();
					newReverseKey.append(node).append("/").append(reverseKey);
					reverseKey = newReverseKey;
				}
				StringBuilder finalreverseKey = new StringBuilder();
				finalreverseKey.append(cycle.get(0)).append("/").append(reverseKey);
				if(cycleMap.get(key.toString()) == null && cycleMap.get(finalreverseKey.toString()) == null){
					cycleMap.put(key.toString(), cycle);
